...
def get_window():
    ...
    tab_1 = [[sg.Text("Choose Template", size=(15, 1)), sg.Combo(template_name_list, size=(45, 1), key='-TEMPLATE_CHOICE-')],
             [sg.Text("Image Source", size=(15, 1)), sg.InputText(key='-FOLDER1-'), sg.FolderBrowse(target='-FOLDER1-')],
             [sg.Button("Start", size=(10, 1), key="-START_OCR-"), sg.Text("", key="-OCR_TAB_OUTPUT-")]]
    ...
    tab_2 = [[sg.Text("Choose Image", size=(15, 1)), sg.InputText(key='-FILE2-'), sg.FileBrowse(target='-FILE2-')],
             ...
             [sg.Button(button_text="Start", size=(10, 1), key="-start_templates-"), sg.Text("", key="-TEMPLATE_TAB_OUTPUT-")]]
...
def main():
    ...
    while True:
        ...
        if state == "RUN_TEMPLATE" and values['-FILE2-'] != "":
            ...
            run_ocr(values["-FOLDER1-"], values["-TEMPLATE_CHOICE-"], window)
            state = "WAITING"
